...
import cucumber.testit.exporter.exporter.model.cucumber.CucumberElement;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberElementType;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberReport;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberScenario;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberStep;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberStepRunStatus;
import cucumber.testit.exporter.exporter.model.cucumber.CucumberTag;
import cucumber.testit.exporter.exporter.model.testit.Autotest;
import cucumber.testit.exporter.exporter.model.testit.AutotestChangeData;
import cucumber.testit.exporter.exporter.model.testit.TestRun;
import cucumber.testit.exporter.exporter.model.testit.TestRunData;
import cucumber.testit.exporter.exporter.model.testit.TestRunResult;
import cucumber.testit.exporter.exporter.model.testit.TestStepResult;
...
public class CucumberReportTransformer {
    ...
    private CucumberStepRunStatus calculateStepRunStatus(CucumberElement cucumberElement) {
        for (CucumberStep step : cucumberElement.getSteps()) {
            if (!step.getResult().getStatus().equals(CucumberStepRunStatus.PASSED)) {
                return CucumberStepRunStatus.FAILED;
            }
        }
        return CucumberStepRunStatus.PASSED;
    }
    ...
    private String convertToOutcome(CucumberElement cucumberElement) {
        ...
        if (status.equals(CucumberStepRunStatus.PASSED)) {
            outcome = CucumberStepRunStatus.PASSED.getValue();
        } else if (status.equals(CucumberStepRunStatus.FAILED)) {
            outcome = CucumberStepRunStatus.FAILED.getValue();
        } else if (status.equals(CucumberStepRunStatus.SKIPPED)) {
            outcome = CucumberStepRunStatus.SKIPPED.getValue();
        } else {
            log.warn("Unrecognized step or test result {} was found", status.getValue());
            outcome = CucumberStepRunStatus.UNDEFINED.getValue();
        }
